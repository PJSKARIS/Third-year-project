...
long fract clipgain = 0.3;

volatile int distortion_mode = 0;
...
long fract fuzz(long fract sample) {
    return tanh(FUZZ_GAIN * sample);
}
...
{
    adi_gpio_Init(NULL, NULL, NULL);
    adi_gpio_SetDirection(ADI_GPIO_PORT_C, ADI_GPIO_PIN_2, ADI_GPIO_DIRECTION_INPUT);
}
 ...
 *pREG_SEC0_CCTL0 = 1;                                    // Enable SEC Core Interface (SCI)
 while(my_audio){
	  adi_gpio_GetData(ADI_GPIO_PORT_C, &button_state);
